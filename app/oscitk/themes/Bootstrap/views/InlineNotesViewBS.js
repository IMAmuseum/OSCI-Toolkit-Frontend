OsciTk.views.InlineNotes = OsciTk.views.BaseView.extend({
    ...
    render: function(data) {
        ...
        var $this = this;
        var contentId = data.contentId; // ex: osci-content-1
        var content = $('#' + contentId); // ex: p #osci-content-1 [Object]
        if (contentId) {
            ...
            // var noteJson = note.toJSON(); // contains: {content_id, id, note, paragraph_number, referenceContent, section_id, tags[]}
            // noteJson.referenceContent = content.text().replace(/[/0-9note cite]+/, '');

  			var notePopover = note.get('note') ? note.get('note') : '';
            var notePopoverForm = "<form class=''><textarea>"+ notePopover +"</textarea></form>";
            console.log(notePopoverForm);

            $('#' + contentId).popover({
            	content: notePopoverForm,
            	html: true
            });
